...
try {
  ...
  const subjectSummary = subjectPanel.locator('[data-subject-summary]');
  const subjectList = subjectPanel.locator('[data-subject-list]');
  ...
  assert.equal(await subjectList.getAttribute('tabindex'), '-1');
  assert.equal(await subjectList.getAttribute('aria-label'), '选择科目');
  ...
  assert.equal(await subjectList.locator(':scope > ul > li').count(), 11);
  await subjectSummary.focus();
  await page.keyboard.press('Space');
  assert.equal(await subjectPanel.getAttribute('open'), null);
  assert.equal(await subjectSummary.evaluate(element => document.activeElement === element), true);
