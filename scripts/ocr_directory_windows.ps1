...
$images = Get-ChildItem -LiteralPath (Resolve-Path -LiteralPath $InputDirectory).Path -Filter '*.png' | Sort-Object Name

foreach ($image in $images) {
    $output = [System.IO.Path]::ChangeExtension($image.FullName, '.txt')
    if (Test-Path -LiteralPath $output) {
        continue
    }
    $file = Wait-WinRtResult ([Windows.Storage.StorageFile]::GetFileFromPathAsync($image.FullName)) ([Windows.Storage.StorageFile])
